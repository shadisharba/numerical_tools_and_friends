# python aheadfork.py Simulation-Software-Engineering/Git-Cheat-Sheet-Octocat
...
def get_forks():
    ...
    session.headers.update({'Authorization': 'token ' + GITHUB_API_TOKEN
                            })  # should carry this header for all request
    ...
    if DEBUG:
        print(
            "[*] Github API Rate Limit. Limit: %s, Remaining: %s, Reset: %s" %
            (resp.headers.get("X-RateLimit-Limit"),
             resp.headers.get("X-RateLimit-Remaining"),
             time.strftime(
                 '%Y-%m-%d %H:%M:%S',
                 time.localtime(int(resp.headers.get("X-RateLimit-Reset"))))))
    ...
    branches = session.get(branches_url % REPO,
                           headers={
    ...
    for branch in branches:
        ...
        if branch_name == "master" or branch_name == "main" or branch.get(
                "protected"):
    ...
    if len(primary_branch_options) == 0:
        ...
        primary_branch = input(
            'Please enter the name of the primary branch. Branch list: \n%s\n'
            % branch_names)
    ...
    for fork in forks:
        ...
        resp = session.get(
            compare_url % (fork.get("full_name"), REPO.split("/")[0]),
            headers={'Accept': 'application/vnd.github.v3+json'})
        if resp.status_code != 200:
            print('[-] compare %s failed: %s' %
                  (fork.get("full_name"), resp.text))
            continue
...
if __name__ == "__main__":
    if len(sys.argv) >= 3:
        ...
        print(
            "\teg: python3 %s ab124942710137429ffdac322314701471234411 Ovi3/BurpBeautifier"
            % sys.argv[0])

        print("Usage 2: python3 %s REPO" % sys.argv[0])
        print(
            "\teg: python3 %s Ovi3/BurpBeautifier (will prompt for API key)" %
            sys.argv[0])
        exit(0)
    ...
    try:
        ...
        if forks:
            ...
            forks = sorted(forks,
                           key=lambda i:
                           (i.get("ahead_by"), i.get("pushed_at")),
                           reverse=True)

            print("Done")
            print("%-48s%-30s%-8s%-8s%-8s%-8s" %
                  ("name", "last push", "star", "fork", "ahead", "behind"))
            for fork in forks:
